...
MassCell::MassCell(long id, float x, float y, float mass, float speed, float direction)
        : m_ID(id), m_x(x), m_y(y), m_mass(mass), m_speed(speed), m_direction(direction),
          m_vx(speed * cos(direction)), m_vy(speed * sin(direction)), m_ax(0), m_ay(0) {
    std::cout << "MassCell created with ID: " << m_ID << std::endl;
}

MassCell::MassCell(const MassCell& other)
        : m_ID(other.m_ID), m_x(other.m_x), m_y(other.m_y), m_mass(other.m_mass),
          m_speed(other.m_speed), m_direction(other.m_direction), m_vx(other.m_vx),
          m_vy(other.m_vy), m_ax(other.m_ax), m_ay(other.m_ay) {
